...
@section('scripts')
    <script>
                    ...
                    <div class="flex gap-4 border-b-2 border-gray-300 mb-4 pb-4 relative">
                        ...
                        <div class="flex-1 relative">
                            ...
                            <div class="absolute top-0 right-0 flex gap-3">
                                <a class="text-xs font-light hover:underline cursor-pointer border border-sscr-red px-2 py-1 rounded" onclick="rejectPost(${post.id})">Reject</a>
                                <a class="text-xs font-light hover:underline cursor-pointer border border-sscr-red px-2 py-1 rounded" onclick="approvePost(${post.id})">Approve</a>
                            </div>
                        </div>
                    </div>
        ...
        async function getPosts() {
            ...
            const url = "{{ route('api.post-approvals.index') }}";
        ...
        async function approvePost(postId) {
            ...
            const url = `/api/post-approvals/${postId}`;
            try {
                const response = await fetch(url, {
                    method: 'POST',
                    body: formData,
