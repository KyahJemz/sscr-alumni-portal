    ...
    @if ($post)
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8   ">
                <div class="overflow-hidden sm:rounded-lg max-w-2xl mx-auto space-y-4 bg">
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('click', function(event) {
            const options = document.querySelectorAll('[id^="post-options-"]');
            options.forEach(option => {
                if (!option.contains(event.target) && !event.target.closest('button')) {
...
<script>
        function PostTemplate(post, user) {
            ...
            const type = `${post.type.charAt(0).toUpperCase() + post.type.slice(1)} on ${new Date(post.created_at).toLocaleString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
                hour: 'numeric',
...
` : '';
            ...
            const likeSection = `
    <div class="flex justify-around items-center mt-4 text-gray-700 border-y-2 border-gray-300 py-2">
        ${userHasLiked ? `
                    <form onsubmit="deleteLike(event)" class="flex items-center gap-2 text-sscr-red">
                        ...
                        <div class="text-sm text-gray-600">${post.likes.length > 1 ? "you and " + (post.likes.length - 1) + " others" : "you"} liked</div>
                    </form>
                ` : `
                    <form onsubmit="sendLike(event)" class="flex items-center gap-2 text-sscr-red">
                        ...
                        <div class="text-sm text-gray-600">${post.likes.length} Likes</div>
                    </form>
                `}
        <div class="flex items-center gap-2 text-sscr-red text-sm"">
...
`;
    ...
    <div class="mt-4 space  -y-2">
        <div class="bg-gray-100 p-2 rounded-md shadow-md flex gap-2 items-start mb-2">
                ...
                <div class="flex gap-4 border-b-2 border-gray-300 mb-4 pb-4 relative">
                    ...
                    ${post.posted_by.id === user.id || user.role === 'cict_admin' || user.role === 'alumni_coordinator' ? `
                            <div class="hidden absolute top-7 right-0 border border-gray-300 dark:border-gray-700 rounded-md px-4 py-2 space-y-2 bg-white"
                                ...
                                <div href="{{ route('posts.edit', $post->id) }}" class="text-sm font-light cursor-pointer">Edit</div>
                                <div href="{{ route('posts.destroy', $post->id) }} class="text-sm font-light cursor-pointer">Delete</div>
                            </div>
                    ` : ''}
                </div>
        ...
        async function getPosts() {
            ...
            const url = "{{ route('api.posts.show', ['id' => $post->id]) }}";
            ...
            try {
                ...
                const postElement = PostTemplate(json.data.post, json.data.user);
