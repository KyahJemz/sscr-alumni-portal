                ...
                <div class="py-1">
                    @forelse ($batches as $batch)
                        <label class="flex items-center px-4 py-2 text-xs">
                            <input type="checkbox" onchange="filterMembers(event)" value="{{$batch->batch}}" class="group-members-batch-filter mr-2 text-sscr-red border-gray-300 focus:ring-0 hover:ring-0 active:ring-0" />
                            {{$batch->batch}}
                        </label>
                    @empty
            ...
            <div id="course-filter-dropdown" class="dropdown absolute z-10 hidden bg-white border border-gray-300 rounded-md mt-1 w-48 shadow-lg">
                <div class="py-1">
                    @forelse ($courses as $course)
                        <label class="flex items-center px-4 py-2 text-xs">
                            <input type="checkbox" onchange="filterMembers(event)" value="{{$course->course}}" class="group-members-course-filter mr-2 text-sscr-red border-gray-300 focus:ring-0 hover:ring-0 active:ring-0" />
                            {{$course->course}}
                        </label>
                    @empty
