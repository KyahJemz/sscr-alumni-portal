                    ...
                    <a class="lg:col-span-2" href="{{ route('news.show', ['post' => $news[0]->id]) }}">
                        <div class="relative">
                            ...
                            <div class="absolute bottom-0 bg-gray-900/50 text-white p-4 w-full">
                                ...
                                <p class="text-sm">{{ \Carbon\Carbon::parse($news[0]->approved_at, 'Asia/Manila')->diffForHumans() }}</p>
                            </div>
                        </div>
                    </a>
                @endif
                    ...
                    @forelse ($news->slice(1, 2) as $newsItem)
                        <a class="flex border border-gray-300 rounded-md p-4" href="{{ route('news.show', ['post' => $newsItem->id]) }}">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($newsItem->approved_at, 'Asia/Manila')->diffForHumans() }}</p>
                            </div>
                        </a>
                    @empty
